-- ============================================================
-- FEED KRİTİK YOLU RPC — Engel + görünürlük + metrik + anket
-- tek sorguda ve ilgili post verilerinin JSON şekli uygulama
-- çıktısıyla birebir özdeş olur.
...
from (
  select
    ...
    || jsonb_build_object(
      ...
      -- 4) Kullanıcıya özel beğeni/bookmark alanları
      'liked_by_me', exists (select 1 from likes l where l.post_id = v.id and l.user_id = p_me),
      -- my_reaction: NULL ise 'like' varsayılan; hiç beğeni yoksa NULL
      ...
      -- 5) Anket (varsa): {id, options:[{id,text,votes,pct}], total_votes, my_vote}
      'poll', (
